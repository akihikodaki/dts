...
1. Check Vhost tx offload (TSO) function by verifying the TSO/cksum in the TCP/IP stack and UFO/cksum
in the IP stack with vm2vm split ring and packed ring vhost-user/virtio-net mergeable path.
...
2. Launch VM1 and VM2,set TSO on in qemu command::
...
3. Check TSO offload status on for the Virtio-net driver on VM1 and VM2::
...
2. Launch VM1 and VM2 with qemu,set TSO on in qemu command::
