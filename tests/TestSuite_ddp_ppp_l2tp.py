...
class TestDdpPppL2tp(TestCase):
    ...
    def ppp_l2tp_pkts(self, flowtype, keyword):
        ...
        if flowtype == 23:
            pkts = {
                "IPV4/L2TP/IPV4/UDP": 'Ether()/IP()/UDP(sport=1701,dport=1701)/PPP_L2TP(proto=0x0021,session_id=%s)/IP(src="%s",dst="%s")/UDP(sport=%d, dport=%d)/Raw("X"* 20)'
                % (session_id, src_ip, dst_ip, sport, dport)
            }
        if flowtype == 24:
            pkts = {
                "IPV4/L2TP/IPV6/UDP": 'Ether()/IP()/UDP(sport=1701, dport=1701)/PPP_L2TP(proto=0x0057,session_id=%s)/IPv6(src="%s", dst="%s")/UDP(sport=%d, dport=%d)/Raw("X"* 20)'
                % (session_id, src_ipv6, dst_ipv6, sport, dport)
            }
        if flowtype == 26:
            pkts = {
                "IPV4/L2TP": 'Ether(src="%s", dst="%s")/IP()/UDP(dport=1701, sport=1701)/L2TP(session_id=%s)/Raw("X"*20)'
                % (src_mac, dst_mac, session_id)
            }
        if flowtype == 28:
            pkts = {
                "PPPOE/IPV4/UDP": 'Ether()/PPPoE(sessionid=%s)/PPP(b\'\\x00\\x21\')/IP(src="%s",dst="%s")/UDP(sport=%d,dport=%d)/Raw("X"*20)'
                % (session_id, src_ip, dst_ip, sport, dport)
            }
        if flowtype == 29:
            pkts = {
                "PPPOE/IPV6/UDP": 'Ether()/PPPoE(sessionid=%s)/PPP(b\'\\x00\\x57\')/IPv6(src="%s",dst="%s")/UDP(sport=%d,dport=%d)/Raw("X"*20)'
                % (session_id, src_ipv6, dst_ipv6, sport, dport)
            }
        if flowtype == 30:
            pkts = {
                "PPPOE": 'Ether(src="%s", dst="%s")/PPPoE(sessionid=%s)'
                % (src_mac, dst_mac, session_id)
            }
        return pkts
    ...
    def raw_packet_generate(self, flowtype):
        ...
        if flowtype == 23:
            a = (
                Ether()
                / IP()
                / UDP(dport=1701, sport=1701)
                / PPP_L2TP(proto=0x0021, session_id=0x7)
                / IP(dst="1.1.1.1", src="2.2.2.2")
                / UDP(dport=4000, sport=8000)
            )
        if flowtype == 24:
            a = (
                Ether()
                / IP()
                / UDP(dport=1701, sport=1701)
                / PPP_L2TP(proto=0x0057, session_id=0x7)
                / IPv6(
                    dst="1001:0db8:85a3:0000:0000:8a2e:0370:0001",
        ...
        if flowtype == 26:
            a = (
                Ether(dst="3C:FD:FE:A3:A0:01", src="4C:FD:FE:A3:A0:01")
                / IP()
                / UDP(dport=1701, sport=1701)
                / L2TP(session_id=0x7)
                / Raw("X" * 20)
        ...
        if flowtype == 30:
            a = Ether(dst="3C:FD:FE:A3:A0:01", src="4C:FD:FE:A3:A0:01") / PPPoE(
                sessionid=0x7
            )
