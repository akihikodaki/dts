...
class TestFlowClassify(TestCase):
    ...
    def get_stream_rule_priority(self, stream_type):
        stream_types = {"UDP_1": 0, "UDP_2": 1, "TCP_1": 2, "TCP_2": 3, "SCTP_1": 4}
    ...
    def run_traffic(self, config):
        ...
        ports_topo = {
            ...
            "stream configs": {
                "count": total_packets,
                "frame config": {"gapUnit": "gapMilliSeconds", "ifg": gap},
                "flow_type": flow_type,
            },
        }
    ...
    def check_filter_pkts(self, log, rule_priority):
        pat = (
            "rule\[{0}\] count=(\d+)".format(rule_priority)
        ...
        if content:
            ...
            total = (
                reduce(lambda x, y: x + y, [int(i) for i in grp])
                if grp and len(grp)
                else 0
            )
        return total
    ...
    def check_test_result(self, config):
        ...
        for stm_type in stm_types:
            ...
            captured_pkts = self.check_filter_pkts(self.test_data, rule_priority)
            self.logger.info(
                "%s %d %d" % (stm_type, rule_priority or 0, captured_pkts or 0)
            )
            msg = None
            if flow_type == "multi_stream":
                ...
                if rule_priority and captured_pkts % total_packets != 0:
                    msg = (
                        "captured packets are not multiples of "
                ...
                if rule_priority is None and captured_pkts != 0:
                    ...
                    msg = "expect {0} ".format(total_packets) + "captured {0}".format(
                        captured_pkts
                    )
                else:
                    continue
    ...
    def verify_traffic(self, stm_types=None, gap=10, flow_type="single_stream"):
