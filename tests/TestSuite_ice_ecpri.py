...
pkt_lst = [
    "Ether(dst='{}')/IP()/UDP(dport={})/Raw('\\x10\\x00')",
...
over_eth_header_packets = {
    "match": [
        "Ether(dst='00:11:22:33:44:11', type=0xAEFE)/Raw('\\x10\\x00\\x02\\x24\\x23\\x45')"
    ],
    "unmatched": [
        "Ether(dst='00:11:22:33:44:11', type=0xAEFE)/Raw('\\x10\\x00\\x02\\x24\\x23\\x46')"
    ],
}
...
over_ip_udp_header_packets = {
    "match": [
        "Ether(dst='00:11:22:33:44:11')/IP()/UDP(dport=0x5123)/Raw('\\x10\\x00\\x02\\x24\\x23\\x45')"
    ],
    "unmatched": [
        "Ether(dst='00:11:22:33:44:11')/IP()/UDP(dport=0x5123)/Raw('\\x10\\x00\\x02\\x24\\x23\\x46')"
    ],
}
...
class TestICEEcpri(TestCase):
    ...
    def set_up(self):
        ...
        self.new_session.send_expect(
            "ip link set {} vf 0 trust on".format(self.pf_interface), "# ", timeout=10
        )
        self.new_session.send_expect(
            "ip link set {} vf 0 mac 00:11:22:33:44:00".format(self.pf_interface),
            "# ",
            timeout=10,
        )
        self.launch_testpmd()
            ...
            try:
                for i in range(len(self.sriov_vfs_port)):
                    ...
                    self.dut.send_expect(
                        "ip link set %s vf %s mac %s"
                        % (self.pf_interface, i, Mac_list[i]),
                        "# ",
                    )
    ...
    def launch_testpmd(self):
        eal_param = " -a {},cap=dcf -a {} -a {}".format(
    ...
    def test_eCPRI_port_config_when_DCF_exit_reset(self):
        ...
        self.new_session.send_expect(
            "ip link set {} vf 0 mac {}".format(self.pf_interface, new_mac),
            "#",
            timeout=10,
        )
        self.send_and_verify(Mac_list[1], self.right_ecpri, if_match=False)
        ...
        self.new_session.send_expect(
            "ip link set {} vf 0 trust off".format(self.pf_interface), "#", timeout=10
        )
        self.send_and_verify(Mac_list[1], self.right_ecpri, if_match=False)
    ...
    def test_negative_eCPRI_port_config_in_DCF(self):
        ...
        for ecpri in ecpri_and_expect_dic.keys():
            out = self.pmd_output.execute_cmd(
                "port config {} udp_tunnel_port add ecpri {}".format(
                    ecpri, self.right_ecpri
                )
            )
            self.verify(ecpri_and_expect_dic[ecpri] in out, "test fail")
        # set an invalid ecpri to test
        ecpri_and_expect_dic = {"ffff": "Bad arguments", "65536": "Bad arguments"}

        for ecpri in ecpri_and_expect_dic.keys():
            out = self.pmd_output.execute_cmd(
                "port config 0 udp_tunnel_port add ecpri {}".format(ecpri)
            )
            self.verify(ecpri_and_expect_dic[ecpri] in out, "test fail")
            if ecpri == "0":
                # test remove an invalid ecpri
                out = self.pmd_output.execute_cmd(
                    "port config 0 udp_tunnel_port rm ecpri {}".format(ecpri)
                )
    ...
    def test_rss_for_udp_ecpri(self):
        ...
        self.pmd_output.execute_cmd(
            "flow validate 1 ingress pattern eth / ipv4 / udp / ecpri common type iq_data / "
            "end actions rss types ecpri end key_len 0 queues end / end"
        )
        self.pmd_output.execute_cmd(
            "flow create 1 ingress pattern eth / ipv4 / udp / ecpri common type iq_data / "
            "end actions rss types ecpri end key_len 0 queues end / end"
        )
        tag_lst = ["x45", "x46", "x47"]
        ...
        self.verify(
            ...
            "test fail, rule still worked.",
        )

    def test_rss_for_eth_ecpri(self):
        self.dut.send_expect("quit", "# ")
        eal_param = " -a {} -a {}".format(
            self.sriov_vfs_port[0].pci, self.sriov_vfs_port[1].pci
        )
        param = " --rxq=16 --txq=16"
        ...
        self.pmd_output.execute_cmd("start")
        self.pmd_output.execute_cmd(
            "flow validate 1 ingress pattern eth / ecpri common type iq_data / "
            "end actions rss types ecpri end key_len 0 queues end / end"
        )
        self.pmd_output.execute_cmd(
            "flow create 1 ingress pattern eth / ecpri common type iq_data / end actions "
            "rss types ecpri end key_len 0 queues end / end"
        )
        tag_lst = ["x45", "x46", "x47"]
        pkt_str = (
            "Ether(dst='{}', type=0xAEFE)/Raw('\\x10\\x00\\x02\\x24\\x23\\%s')".format(
                Mac_list[1]
            )
        )
        data_lst = self.get_receive_lst(tag_lst, [pkt_str])
    ...
    def test_rss_multirules_multiports(self):
        ...
        tag_lst = ["x45", "x46"]
        module_pkt_lst = [
            "Ether(dst='{}')/IP()/UDP(dport=0x5123)/Raw('\\x10\\x00\\x02\\x24\\x23\\%s')",
        ...
        for key in out_data.keys():
            ...
            self.verify(
                len(set(hash_lst)) == 2 and None not in hash_lst,
                "test fail, RSS hash is same.",
            )
    ...
    def test_rss_without_or_with_udp_port_set_for_udp_ecpri_rule(self):
        ...
        pkt = "Ether(dst='{}')/IP()/UDP(dport=0x5123)/Raw('\\x10\\x00\\x02\\x24\\x23\\%s')".format(
            Mac_list[1]
        )
        self.pmd_output.execute_cmd(
            "flow create 1 ingress pattern eth / ipv4 / udp / ecpri common type iq_data / end "
    ...
    def test_DCF_reset_for_udp_ecpri_rss(self):
        ...
        self.new_session.send_expect(
            "ip link set {} vf 0 mac 00:11:22:33:44:66".format(self.pf_interface), "#"
        )
        out_data = self.get_receive_lst(tag_lst, [pkt])
        # verify
        hash_lst = [i.get("RSS hash") for i in out_data]
        ...
        self.launch_testpmd()
        self.new_session.send_expect(
            "ip link set {} vf 0 trust off".format(self.pf_interface), "#"
        )
        out_data = self.get_receive_lst(tag_lst, [pkt])
        # verify
        hash_lst = [i.get("RSS hash") for i in out_data]
    ...
    def test_DCF_reset_for_eth_ecpri_rss(self):
        ...
        self.pmd_output.execute_cmd("port config 0 udp_tunnel_port add ecpri 0x5123")
        self.pmd_output.execute_cmd(
            "flow create 1 ingress pattern eth / ecpri common type iq_data / end actions rss types"
            " ecpri end key_len 0 queues end / end"
        )
        ...
        hash_lst = [i.get("RSS hash") for i in out_data]
        self.verify(
            len(set(hash_lst)) == 3 and None not in hash_lst,
            "test fail, RSS hash is same",
        )
    ...
    def test_DCF_exit_for_eth_ecpri_and_udp_ecpri_rss(self):
        ...
        eal_param1 = " -a {} -a {}".format(
            self.sriov_vfs_port[1].pci, self.sriov_vfs_port[2].pci
        )
        param = " --rxq=16 --txq=16"
        pmd_output1.start_testpmd(
            cores=list(range(8)),
            eal_param=eal_param1,
            param=param,
            prefix="test2",
            socket=self.ports_socket,
        )
        pmd_output1.execute_cmd(
            "flow create 0 ingress pattern eth / ipv4 / udp / ecpri common type iq_data / end "
            "actions rss types ecpri end key_len 0 queues end / end"
        )
        pmd_output1.execute_cmd(
            "flow create 1 ingress pattern eth / ecpri common type iq_data / end actions rss "
            "types ecpri end key_len 0 queues end / end"
        )
        ...
        tag_lst = ["x45", "x46"]
        pkt_lst = [
            "Ether(dst='{}')/IP()/UDP(dport=0x5123)/Raw('\\x10\\x00\\x02\\x24\\x23\\%s')".format(
                Mac_list[1]
            ),
            "Ether(dst='{}', type=0xAEFE)/Raw('\\x10\\x00\\x02\\x24\\x23\\%s')".format(
                Mac_list[2]
        ...
        hash_lst = [i.get("RSS hash") for i in out_data]
        self.verify(
            len(set(hash_lst)) == 4 and None not in hash_lst,
            "test fail, Rss hash is same.",
        )
        self.pmd_output.execute_cmd("quit", "#")
        out_data = self.get_receive_lst(tag_lst, pkt_lst, pmd_output1)
        ...
        hash_lst = [i.get("RSS hash") for i in out_data]
        self.verify(
            hash_lst[0] == hash_lst[2] and hash_lst[1] != hash_lst[3],
            "test fail, hash value is wrong.",
        )
    ...
    def check_fdir_rule(self, port_id=0, stats=True, rule_list=None):
        ...
        if stats:
            ...
            if rule_list:
                ...
                result = [i.group(1) for i in res]
                self.verify(
                    not [i for i in rule_list if i in result],
                    "check rule list failed. flow rule %s on port %s is existed"
                    % (rule_list, port_id),
                )
            else:
    ...
    def _rte_flow_validate(self, vectors):
        ...
        for tv in vectors:
            try:
                count = 1
                port_id = (
                    tv["send_port"]["port_id"]
                    if tv["send_port"].get("port_id") is not None
                    else 0
                )
                dut_port_id = (
                ...
                self.check_fdir_rule(
                    port_id=tv["check_param"]["port_id"], rule_list=["0"] + rule_li
                )
                ...
                self.check_fdir_rule(
                    port_id=dut_port_id, rule_list=rule_li, stats=False
                )
    ...
    def test_ecpri_fdir_multirules(self):
        ...
        self.verify(
            [data.get("FDIR matched ID") for data in data_lst] == ["0x0"],
            "pkt has wrong mark id!",
        )
        data_lst = self.get_receive_lst(tag_lst[1:], pkt_lst)
        self.verify(
            [data.get("FDIR matched ID") for data in data_lst] == ["0x1"],
            "pkt has wrong mark id!",
        )
        pkt_lst = [
            "Ether(dst='{}', type=0xAEFE)/Raw('\\x10\\x00\\x02\\x24\\x23\\%s')".format(
    ...
    def test_ecpri_fdir_negative_case(self):
        out = self.pmd_output.execute_cmd(
            "flow create 1 ingress pattern eth / ipv4 / udp / ecpri common type iq_data pc_id is "
            "0x2345 / end actions rss queues 5 6 end / mark id 0 / end"
        )
        self.verify(
            "Failed to create parser engine.: Invalid argument" in out,
            "test fail, bad rule set success.",
        )
        out = self.pmd_output.execute_cmd("flow list 1")
        r = r"flow list 1(\s+)(.*)"
        m = re.match(r, out)
    ...
    def test_ecpri_fdir_when_DCF_reset(self):
        self.pmd_output.execute_cmd("port config 0 udp_tunnel_port add ecpri 0x5123")
        self.pmd_output.execute_cmd(
            "flow create 1 ingress pattern eth / ipv4 / udp / ecpri common type iq_data pc_id "
            "is 0x2345 / end actions queue index 1 / mark id 1 / end"
        )
        self.pmd_output.execute_cmd(
            "flow create 1 ingress pattern eth / ecpri common type iq_data pc_id is "
        ...
        # verify
        self.verify(
            [data.get("queue") for data in data_lst] == ["1", "2"],
            "pkt to the wrong queue!",
        )
        self.verify(
            [data.get("FDIR matched ID") for data in data_lst] == ["0x1", "0x2"],
            "pkt with wrong FDIR matched ID!",
        )
        self.new_session.send_expect(
            "ip link set {} vf 0 mac 00:11:22:33:44:66".format(self.pf_interface), "#"
        )
    ...
    def test_ecpri_fdir_when_DCF_exit(self):
        ...
        pmd_output1.execute_cmd(
            "flow create 0 ingress pattern eth / ipv4 / udp / ecpri common type iq_data pc_id "
            "is 0x2345 / end actions queue index 1 / mark id 1 / end"
        )
        pmd_output1.execute_cmd(
            "flow create 0 ingress pattern eth / ecpri common type iq_data pc_id is 0x2345 / end "
        ...
        self.verify(
            [data.get("FDIR matched ID") for data in data_lst] == ["0x1", "0x2"]
            and [data.get("queue") for data in data_lst] == ["1", "2"],
    ...
    def compile_dpdk(self):
        cmd_lst = [
            r"sed -i '/iavf_flex_rxd_to_vlan_tci(rxm, &rxd, rxq->rx_flags);/i\printf(\"++++++++++++ptype=%u\\n\",IAVF_RX_FLEX_DESC_PTYPE_M & rte_le_to_cpu_16(rxd.wb.ptype_flex_flags0));' ",
    ...
    def send_and_verify(self, dts_mac, ecpri, if_match=True):
        ...
        for i in range(len(pkt_lst)):
            ...
            self.verify(
                ptype_lst[i] in out, "ptype is error, expect {}".format(ptype_lst[i])
            )

    def send_pkt(
        self, pkt_str="", dts_mac="00:11:22:33:44:11", ecpri="0x5123", pmd_output=""
    ):
