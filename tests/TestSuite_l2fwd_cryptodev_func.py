...
class TestL2fwdCrypto(TestCase):
    ...
    def __calculate_total_cases_numb(self):
        ...
        for index, value in list(test_vectors.items()):
            test_vector_list = self.__test_vector_to_vector_list(
                value, core_mask="-1", port_mask=self.port_mask
            )
        ...
        for k, v in list(alg_map.items()):
            self.logger.info(
                "Total {name} cases:\t\t\t{number}".format(name=k, number=v)
            )
    ...
    def __execute_l2fwd_crypto_test(self, test_vectors, test_vector_name):
        ...
        if (
            running_case
            in self._suite_result.internals[dut_index + 1][target_index + 2][
                suite_index + 1
            ]
        ):
        ...
        for test_vector in test_vector_list:
            ...
            self.vector = []
            cmd_str = self.__test_vector_to_cmd(
                test_vector, core_mask=self.core_mask, port_mask=self.port_mask
            )
            ...
            inst = self.tester.tcpdump_sniff_packets(
                self.rx_interface,
                filters=[{"layer": "ether", "config": {"dst": "52:00:00:00:00:01"}}],
            )
            ...
            for i in range(len(pkt_rec)):
                ...
                if cipher_text.lower() == test_vector["output_cipher"].lower():
                    ...
                    if test_vector["output_cipher"] != "":
                        ...
                        self.logger.info(
                            "Ref Cipher text       = " + test_vector["output_cipher"]
                        )
                    else:
                ...
                if hash_length != 0:
                    if test_vector["auth_algo"] == "null":
                        hash_text = str(
                            binascii.b2a_hex(
                                pkt_rec.pktgen.pkt["Raw"].getfieldval("load")
                            ),
                            encoding="utf-8",
                        )
                    else:
                    ...
                    if hash_text.lower() == test_vector["output_hash"].lower():
                        ...
                        self.logger.info(
                            "Ref Hash text       = " + test_vector["output_hash"]
                        )
                else:
    ...
    def __test_vector_to_cmd(self, test_vector, core_mask="", port_mask=""):
        ...
        if test_vector["chain"].upper() == "AEAD":
            ...
            if self.__check_field_in_vector(test_vector, "cipher_key"):
                cipher_key = " --aead_key " + self.__format_hex_to_param(
                    test_vector["cipher_key"]
                )
                self.vector.append(
                    "aead_key_%d" % (len(test_vector["cipher_key"]) // 2)
                )
            ...
            if self.__check_field_in_vector(test_vector, "cipher_key"):
                cipher_key = " --cipher_key " + self.__format_hex_to_param(
                    test_vector["cipher_key"]
                )
                self.vector.append(
                    "cipher_key_%d" % (len(test_vector["cipher_key"]) // 2)
                )
            if self.__check_field_in_vector(test_vector, "iv"):
            ...
            if self.__check_field_in_vector(test_vector, "auth_key"):
                auth_key = " --auth_key " + self.__format_hex_to_param(
            ...
            if self.__check_field_in_vector(test_vector, "auth_key_random_size"):
                auth_key_random_size = (
                    " --auth_key_random_size " + test_vector["auth_key_random_size"]
                )
                self.vector.append(
                    "auth_key_random_size_%d" % len(test_vector["auth_key_random_size"])
                )
            ...
            if self.__check_field_in_vector(test_vector, "aad_random_size"):
                ...
                self.vector.append(
                    "aad_random_size_%d" % len(test_vector["aad_random_size"])
                )
    ...
    def __cryptography_cipher(self, vector):
        ...
        if vector["chain"].upper() != "AEAD":
            ...
            if cipher_str == "":
                ...
                cipher_str = (
                    encryptor.update(binascii.a2b_hex(vector["input"]))
                    + encryptor.finalize()
                )
        else:
            if vector["cipher_algo"] == "aes-gcm":
                aesgcm = AESGCM(key)
                cipher_str = aesgcm.encrypt(
                    iv,
    ...
    def __gen_cipher_output(self, vector):
        ...
        if (
            (((vector["cipher_algo"]).upper()).find("KASUMI") != -1)
            or (((vector["cipher_algo"]).upper()).find("SNOW3G") != -1)
            or (((vector["cipher_algo"]).upper()).find("ZUC") != -1)
        ):
            ...
            cipher_str = (
                vector["input"]
                if vector["chain"].upper().find("HASH_") == -1
                else vector["output_hash"]
            )
        else:
    ...
    def __gen_hash_output(self, vector):
        ...
        if auth_algo == "md5-hmac":
            hash_str = hmac.new(
                binascii.a2b_hex(vector["auth_key"]),
                binascii.a2b_hex(input_str),
                hashlib.md5,
            ).hexdigest()
            ...
            hash_str = aesgcm.encrypt(
                iv, binascii.a2b_hex(vector["input"]), binascii.a2b_hex(vector["aad"])
            )
    ...
    def __is_valid_size(self, key_type, algo, size):
        algo_size_map = {
            ...
            "aes-ctr": {"cipher_key": [16, 24, 32], "iv": [12, 16]},
            "3des-cbc": {"cipher_key": [8, 16, 24], "iv": [8]},
            "3des-ctr": {"cipher_key": [16, 24], "iv": [8]},
            "aes-gcm": {
                "cipher_key": [16, 24, 32],
                "aad": [
                    0,
            ...
            "snow3g-uea2": {"cipher_key": [16], "iv": [16]},
            "kasumi-f8": {"cipher_key": [16], "iv": [8]},
            "zuc-eea3": {"cipher_key": [16], "iv": [16]},
            "null": {"cipher_key": [0], "auth_key": [0], "aad": [0], "iv": [0]},
            "md5-hmac": {"auth_key": [64], "aad": [0], "digest_size": [12, 16]},
            "sha1-hmac": {"auth_key": [64], "aad": [0], "digest_size": [12, 20]},
            "sha2-224-hmac": {"auth_key": [64], "aad": [0], "digest_size": [14, 28]},
            "sha2-256-hmac": {"auth_key": [64], "aad": [0], "digest_size": [16, 32]},
            "sha2-384-hmac": {"auth_key": [128], "aad": [0], "digest_size": [24, 48]},
            "sha2-512-hmac": {"auth_key": [128], "aad": [0], "digest_size": [32, 64]},
            "aes-xcbc-mac": {"auth_key": [16], "aad": [0], "digest_size": [12, 16]},
            "aes-gmac": {
                "auth_key": [16, 24, 32],
                "aad": [1, 12, 16, 64, 128, 256, 65535],
                "digest_size": [8, 12, 16],
            },
            "snow3g-uia2": {"auth_key": [16], "aad": [16], "digest_size": [4]},
            "kasumi-f9": {"auth_key": [16], "aad": [8], "digest_size": [4]},
            "zuc-eia3": {"auth_key": [16], "aad": [16], "digest_size": [4]},
            "md5": {"auth_key": [0], "aad": [0], "digest_size": [16]},
            "sha1": {"auth_key": [0], "aad": [0], "digest_size": [20]},
            "sha2-224": {"auth_key": [0], "aad": [0], "digest_size": [28]},
            "sha2-256": {"auth_key": [0], "aad": [0], "digest_size": [32]},
            "sha2-384": {"auth_key": [0], "aad": [0], "digest_size": [48]},
            "sha2-512": {"auth_key": [0], "aad": [0], "digest_size": [64]},
        }
        result = False
        ...
        if test_vector["chain"] == "HASH_ONLY":
            ...
            for cipher_op in cipher_op_list:
                if self.__is_valid_op(test_vector["chain"], cipher_op):
                    ...
                    self.__iter_cipher_key(
                        test_vector, vector_list, core_mask, port_mask
                    )
        ...
        if test_vector["chain"] == "HASH_ONLY":
            ...
            for cipher_key in cipher_key_list:
                ...
                if isinstance(cipher_key, int):
                    if self.__is_valid_size(
                        "cipher_key", test_vector["cipher_algo"], cipher_key
                    ):
        ...
        if test_vector["chain"] == "HASH_ONLY":
            ...
            for iv in iv_list:
                ...
                if isinstance(iv, int):
                    if self.__is_valid_size("iv", test_vector["cipher_algo"], iv):
                        test_vector["iv"] = self.__gen_key(iv)
                        self.__iter_auth_algo(
                            test_vector, vector_list, core_mask, port_mask
                        )
                    else:
                        continue
        ...
        if test_vector["chain"] in ["CIPHER_ONLY", "AEAD"]:
            ...
            for auth_key in auth_key_list:
                ...
                if isinstance(auth_key, int):
                    if self.__is_valid_size(
                        "auth_key", test_vector["auth_algo"], auth_key
                    ):
            ...
            for aad in aad_list:
                ...
                if isinstance(aad, int):
                    if self.__is_valid_size("aad", test_vector["auth_algo"], aad) or (
                        test_vector["chain"] == "AEAD"
                        and self.__is_valid_size("aad", test_vector["cipher_algo"], aad)
                    ):
        ...
        if test_vector["chain"] == "CIPHER_ONLY":
            ...
            for digest in digest_list:
                ...
                if isinstance(digest, int):
                    if self.__is_valid_size(
                        "digest_size", test_vector["auth_algo"], digest
                    ) or (
                        test_vector["chain"] == "AEAD"
                        and self.__is_valid_size(
                            "digest_size", test_vector["cipher_algo"], digest
                        )
                    ):
