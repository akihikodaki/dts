...
tv_MAC_IPV4_L2TPv3_chksum = {
    "good_checksum": "Ether(dst='%s')/IP(src='192.168.0.3', proto=115)/L2TP(b'\\x00\\x00\\x00\\x11')/Raw('X'*480)"
    % vf0_mac,
    "bad_checksum": "Ether(dst='%s')/IP(src='192.168.0.3', proto=115, chksum=0x1234)/L2TP(b'\\x00\\x00\\x00\\x11')/Raw('X'*480)"
    % vf0_mac,
}
tv_MAC_IPV4_ESP_chksum = {
    "good_checksum": 'Ether(dst="%s")/IP(src="192.168.0.3", proto=50)/ESP(spi=11)/Raw("X"*480)'
    % vf0_mac,
    "bad_checksum": 'Ether(dst="%s")/IP(src="192.168.0.3", proto=50, chksum=0x1234)/ESP(spi=11)/Raw("X"*480)'
    % vf0_mac,
}
tv_MAC_IPV4_AH_chksum = {
    "good_checksum": 'Ether(dst="%s")/IP(src="192.168.0.3", proto=50)/AH(spi=11)/Raw("X"*480)'
    % vf0_mac,
    "bad_checksum": 'Ether(dst="%s")/IP(src="192.168.0.3", proto=50, chksum=0x1234)/AH(spi=11)/Raw("X"*480)'
    % vf0_mac,
}
tv_MAC_IPV4_NAT_T_ESP_chksum = {
    "good_ip_udp_checksum": 'Ether(dst="%s")/IP(src="192.168.0.20")/UDP(dport=4500)/ESP(spi=2)/Raw("x"*480)'
    % vf0_mac,
    "bad_ip_good_udp_checksum": 'Ether(dst="%s")/IP(src="192.168.0.20", chksum=0x1234)/UDP(dport=4500)/ESP(spi=2)/Raw("x"*480)'
    % vf0_mac,
    "bad_ip_udp_checksum": 'Ether(dst="%s")/IP(src="192.168.0.20", chksum=0x1234)/UDP(dport=4500,chksum=0x1234)/ESP(spi=2)/Raw("x"*480)'
    % vf0_mac,
    "bad_udp_good_ip_checksum": 'Ether(dst="%s")/IP(src="192.168.0.20")/UDP(dport=4500,chksum=0x1234)/ESP(spi=2)/Raw("x"*480)'
    % vf0_mac,
}
tv_MAC_IPV6_NAT_T_ESP_chksum = {
    "good_udp_checksum": 'Ether(dst="%s")/IPv6(src="1111:2222:3333:4444:5555:6666:7777:8888")/UDP(dport=4500)/ESP(spi=2)/Raw("x"*480)'
    % vf0_mac,
    "bad_udp_checksum": 'Ether(dst="%s")/IPv6(src="1111:2222:3333:4444:5555:6666:7777:8888")/UDP(dport=4500,chksum=0x1234)/ESP(spi=2)/Raw("x"*480)'
    % vf0_mac,
}

tv_MAC_IPV4_L2TPv3_vlan = {
    "matched vlan": "Ether(dst='%s')/Dot1Q(vlan=1)/IP(proto=115)/L2TP(b'\\x00\\x00\\x00\\x11')/Raw('x'*480)"
    % vf0_mac,
    "dismatched vlan": "Ether(dst='%s')/Dot1Q(vlan=2)/IP(proto=115)/L2TP(b'\\x00\\x00\\x00\\x11')/Raw('x'*480)"
    % vf0_mac,
    "no vlan": "Ether(dst='%s')/IP(proto=115)/L2TP(b'\\x00\\x00\\x00\\x11')/Raw('x'*480)"
    % vf0_mac,
}
tv_MAC_IPV6_L2TPv3_vlan = {
    "matched vlan": "Ether(dst='%s')/Dot1Q(vlan=1)/IPv6(nh=115)/L2TP(b'\\x00\\x00\\x00\\x11')/Raw('x'*480)"
    % vf0_mac,
    "dismatched vlan": "Ether(dst='%s')/Dot1Q(vlan=2)/IPv6(nh=115)/L2TP(b'\\x00\\x00\\x00\\x11')/Raw('x'*480)"
    % vf0_mac,
    "no vlan": "Ether(dst='%s')/IPv6(nh=115)/L2TP(b'\\x00\\x00\\x00\\x11')/Raw('x'*480)"
    % vf0_mac,
}
tv_MAC_IPV4_ESP_vlan = {
    "matched vlan": "Ether(dst='%s')/Dot1Q(vlan=1)/IP(proto=50)/ESP(spi=1)/Raw('x'*480)"
    % vf0_mac,
    "dismatched vlan": "Ether(dst='%s')/Dot1Q(vlan=2)/IP(proto=50)/ESP(spi=1)/Raw('x'*480)"
    % vf0_mac,
    "no vlan": "Ether(dst='%s')/IP(proto=50)/ESP(spi=1)/Raw('x'*480)" % vf0_mac,
}
tv_MAC_IPV6_ESP_vlan = {
    "matched vlan": "Ether(dst='%s')/Dot1Q(vlan=1)/IPv6(nh=50)/ESP(spi=1)/Raw('x'*480)"
    % vf0_mac,
    "dismatched vlan": "Ether(dst='%s')/Dot1Q(vlan=2)/IPv6(nh=50)/ESP(spi=1)/Raw('x'*480)"
    % vf0_mac,
    "no vlan": "Ether(dst='%s')/IPv6(nh=50)/ESP(spi=1)/Raw('x'*480)" % vf0_mac,
}
tv_MAC_IPV4_AH_vlan = {
    "matched vlan": "Ether(dst='%s')/Dot1Q(vlan=1)/IP(proto=51)/AH(spi=1)/Raw('x'*480)"
    % vf0_mac,
    "dismatched vlan": "Ether(dst='%s')/Dot1Q(vlan=2)/IP(proto=51)/AH(spi=1)/Raw('x'*480)"
    % vf0_mac,
    "no vlan": "Ether(dst='%s')/IP(proto=51)/AH(spi=1)/Raw('x'*480)" % vf0_mac,
}
tv_MAC_IPV6_AH_vlan = {
    "matched vlan": "Ether(dst='%s')/Dot1Q(vlan=1)/IPv6(nh=51)/AH(spi=1)/Raw('x'*480)"
    % vf0_mac,
    "dismatched vlan": "Ether(dst='%s')/Dot1Q(vlan=2)/IPv6(nh=51)/AH(spi=1)/Raw('x'*480)"
    % vf0_mac,
    "no vlan": "Ether(dst='%s')/IPv6(nh=51)/AH(spi=1)/Raw('x'*480)" % vf0_mac,
}
tv_MAC_IPV4_NAT_T_ESP_vlan = {
    "matched vlan": "Ether(dst='%s')/Dot1Q(vlan=1)/IP()/UDP(dport=4500)/ESP(spi=1)/Raw('x'*480)"
    % vf0_mac,
    "dismatched vlan": "Ether(dst='%s')/Dot1Q(vlan=2)/IP()/UDP(dport=4500)/ESP(spi=1)/Raw('x'*480)"
    % vf0_mac,
    "no vlan": "Ether(dst='%s')/IP()/UDP(dport=4500)/ESP(spi=1)/Raw('x'*480)" % vf0_mac,
}
tv_MAC_IPV6_NAT_T_ESP_vlan = {
    "matched vlan": "Ether(dst='%s')/Dot1Q(vlan=1)/IPv6()/UDP(dport=4500)/ESP(spi=1)/Raw('x'*480)"
    % vf0_mac,
    "dismatched vlan": "Ether(dst='%s')/Dot1Q(vlan=2)/IPv6()/UDP(dport=4500)/ESP(spi=1)/Raw('x'*480)"
    % vf0_mac,
    "no vlan": "Ether(dst='%s')/IPv6()/UDP(dport=4500)/ESP(spi=1)/Raw('x'*480)"
    % vf0_mac,
}
...
class TestL2tpEspCoverage(TestCase):
    ...
    def set_up(self):
        ...
        """
        cases_list = [
            "test_MAC_IPV4_L2TPv3_HW_checksum_vlan_strip",
    ...
    def create_testpmd_command(self, port_info, rx_checksum=0):
        ...
        self.pmd_output.start_testpmd(
            cores="1S/8C/1T", param=param_str, eal_param="-a %s" % port_pci
        )
    ...
    def checksum_verify(self, packets_sent):
        ...
        for packet_type in list(packets_sent.keys()):
            self.tester.scapy_append(
                'sendp([%s], iface="%s")' % (packets_sent[packet_type], self.tx_iface)
            )
    ...
    def vlan_strip_insertion_verify(self, packets_sent):
        ...
        self.tester.scapy_append(
            'sendp([%s], iface="%s")' % (packets_sent["matched vlan"], self.tx_iface)
        )
        self.tester.scapy_execute()
        time.sleep(1)
        out = self.pmd_output.execute_cmd("stop")
        ...
        self.dut.send_expect("start", "testpmd> ")
        self.tester.scapy_append(
            'sendp([%s], iface="%s")' % (packets_sent["dismatched vlan"], self.tx_iface)
        )
        self.tester.scapy_execute()
        time.sleep(1)
        out = self.pmd_output.execute_cmd("stop")
        ...
        self.start_tcpdump(self.tx_iface)
        self.tester.scapy_append(
            'sendp([%s], iface="%s")' % (packets_sent["no vlan"], self.tx_iface)
        )
        self.tester.scapy_execute()
        time.sleep(1)
        out = self.pmd_output.execute_cmd("stop")
    ...
    def test_MAC_IPV4_L2TPv3_HW_checksum_vlan_strip(self):
        ...
        rfc.check_iavf_fdir_mark(
            out, pkt_num=1, check_param={"port_id": 0, "queue": 1, "mark_id": 4}
        )
        ...
        rfc.check_iavf_fdir_mark(
            out,
            pkt_num=1,
            check_param={"port_id": 0, "queue": 1, "mark_id": 4},
            stats=False,
        )
        # check the rx checksum
        bad_ipcsum = self.pmd_output.get_pmd_value("Bad-ipcsum:", out)
