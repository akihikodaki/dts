...
class TestPowerBranchRatio(TestCase):
        ...
        pkt_configs = {
            "UDP_1": {
                ...
                "pkt_layers": {
                    ...
                    "raw": {
                        "payload": ["58"]
                        * self.get_pkt_len(pkt_name, frame_size=self.frame_size)
                    },
                },
            },
        }
    ...
    def start_vm_power_mgr(self):
        sub_option = (
            "-v " "-c {core_mask} " "-n {mem_channel} " "-m {memory_size} " "--no-pci "
        ).format(
            **{
        ...
        prompt = "vmpower>"
        option = sub_option + (
            " -- --core-branch-ratio={0}-{1}:{2}".format(
                self.from_core, self.to_core, self.branch_ratio
            )
            if self.branch_ratio
            else ""
        )
    ...
    def init_testpmd(self):
        self.testpmd = os.path.join(self.target_dir, self.dut.apps_name["test-pmd"])
    ...
    def check_core_freq_no_traffic(self, core_index, ref_freq_name):
        ...
        self.logger.info(msg)
        displayFreqData = (
            "Freqs in NO Traffic: Check Core Freq {0} <= Expected Freq {1}".format(
                self.scaling_cur_freq, expected_freq
            )
        )
        self.logger.info(displayFreqData)
