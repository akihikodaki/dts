...
class TestPVPVirtioUserMultiQueuesPortRestart(TestCase):
    ...
    def start_virtio_user_testpmd(self, vdevs, vector_flag=False):
        ...
        if vector_flag:
            ...
            param = "--tx-offloads=0x0 --rss-ip --nb-cores=2 --rxq={} --txq={}".format(
                self.queue_number, self.queue_number
            )
        else:
            param = "--tx-offloads=0x0 --enable-hw-vlan-strip --rss-ip --nb-cores=2 --rxq={} --txq={}".format(
                self.queue_number, self.queue_number
            )
    ...
    def check_packets_of_each_queue(self, frame_size):
        ...
        p = re.compile(
            "RX Port= 0/Queue= (\d+) -> TX Port= 0/Queue= \d+.*\n.*RX-packets:\s?(\d+).*TX-packets:\s?(\d+)"
        )
        res = p.findall(out)
        ...
        self.verify(
            self.res_queues == list(range(self.queue_number)),
            "frame_size: %s, expect %s queues to handle packets, result %s queues"
            % (frame_size, list(range(self.queue_number)), self.res_queues),
        )
        self.verify(
            all(self.res_rx_pkts),
            "each queue should has rx packets, result: %s" % self.res_rx_pkts,
        )
        self.verify(
            all(self.res_tx_pkts),
            "each queue should has tx packets, result: %s" % self.res_tx_pkts,
        )
    ...
    def test_perf_pvp_2queues_test_with_packed_ring_mergeable_path(self):
        ...
        self.start_virtio_user_testpmd(
            vdevs="net_virtio_user0,mac=00:01:02:03:04:05,path=./vhost-net,queues=2,packed_vq=1,mrg_rxbuf=1,in_order=0,queue_size=255"
        )
    ...
    def test_perf_pvp_2queues_test_with_packed_ring_nonmergeable_path(self):
        self.start_vhost_testpmd()
        self.start_virtio_user_testpmd(
            vdevs="net_virtio_user0,mac=00:01:02:03:04:05,path=./vhost-net,queues=2,packed_vq=1,mrg_rxbuf=0,in_order=0,queue_size=255"
        )
    ...
    def test_perf_pvp_2queues_test_with_split_ring_inorder_mergeable_path(self):
        self.start_vhost_testpmd()
        self.start_virtio_user_testpmd(
            vdevs="net_virtio_user0,mac=00:01:02:03:04:05,path=./vhost-net,queues=2,in_order=1,mrg_rxbuf=1"
        )
    ...
    def test_perf_pvp_2queues_test_with_split_ring_inorder_nonmergeable_path(self):
        self.start_vhost_testpmd()
        self.start_virtio_user_testpmd(
            vdevs="net_virtio_user0,mac=00:01:02:03:04:05,path=./vhost-net,queues=2,in_order=1,mrg_rxbuf=0,vectorized=1"
        )
    ...
    def test_perf_pvp_2queues_test_with_split_ring_mergeable_path(self):
        self.start_vhost_testpmd()
        self.start_virtio_user_testpmd(
            vdevs="net_virtio_user0,mac=00:01:02:03:04:05,path=./vhost-net,queues=2,in_order=0,mrg_rxbuf=1"
        )
    ...
    def test_perf_pvp_2queues_test_with_split_ring_nonmergeable_path(self):
        self.start_vhost_testpmd()
        self.start_virtio_user_testpmd(
            vdevs="net_virtio_user0,mac=00:01:02:03:04:05,path=./vhost-net,queues=2,in_order=0,mrg_rxbuf=0,vectorized=1"
        )
    ...
    def test_perf_pvp_2queues_test_with_packed_ring_inorder_mergeable_path(self):
        self.start_vhost_testpmd()
        self.start_virtio_user_testpmd(
            vdevs="net_virtio_user0,mac=00:01:02:03:04:05,path=./vhost-net,queues=2,packed_vq=1,mrg_rxbuf=1,in_order=1,queue_size=255"
        )
    ...
    def test_perf_pvp_2queues_test_with_packed_ring_inorder_nonmergeable_path(self):
        self.start_vhost_testpmd()
        self.start_virtio_user_testpmd(
            vdevs="net_virtio_user0,mac=00:01:02:03:04:05,path=./vhost-net,queues=2,packed_vq=1,mrg_rxbuf=0,in_order=1,vectorized=1,queue_size=255"
        )
