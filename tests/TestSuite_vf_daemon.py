...
class TestVfDaemon(TestCase):
    ...
    def test_multicast_mode(self):
        ...
        self.verify(
            "dst=%s" % self.vf0_mac in out, "Failed to disable vf multicast mode!!!"
        )
        ...
        self.verify(
            "dst=%s" % multi_mac in out, "Failed to enable vf multicast mode!!!"
        )

        out = self.send_and_pmdout(self.vf0_mac)
        self.verify("received" in out, "Failed to enable vf multicast mode!!!")
        self.verify(
            "dst=%s" % self.vf0_mac in out, "Failed to enable vf multicast mode!!!"
        )
    ...
    def test_promisc_mode(self):
        ...
        self.verify(
            "dst=%s" % self.vf0_mac in out, "Failed to disable vf promisc mode!!!"
        )
        ...
        self.verify("dst=%s" % wrong_mac in out, "Failed to enable vf promisc mode!!!")

        out = self.send_and_pmdout(self.vf0_mac)
        self.verify("received" in out, "Failed to enable vf promisc mode!!!")
        self.verify(
            "dst=%s" % self.vf0_mac in out, "Failed to enable vf promisc mode!!!"
        )
    ...
    def test_broadcast_mode(self):
        ...
        self.verify("dst=%s" % dst_mac in out, "Failed to enable vf broadcast mode!!!")
    ...
    def test_vlan_tag(self):
        ...
        for rx_vlan in rx_vlans:
            ...
            self.verify(
                self.vf0_mac.lower() not in out, "Failed to disable vlan tag!!!"
            )

            self.dut_testpmd.execute_cmd("set vf vlan tag 0 0 on")
            time.sleep(3)
            out = self.send_and_macstrip(self.vf0_mac, rx_vlan)
            self.verify(self.vf0_mac.lower() in out, "Failed to enable vlan tag!!!")
    ...
    def test_tx_loopback(self):
        ...
        self.vm1_testpmd.start_testpmd(
            VM_CORES_MASK, "--port-topology=chained --eth-peer=0,%s" % self.vf0_mac
        )
        time.sleep(10)
        self.vm0_testpmd.execute_cmd("set fwd rxonly")
        self.vm0_testpmd.execute_cmd("set verbose 1")
        ...
        self.verify(self.vf0_mac.lower() in dumpout, "Failed to disable tx loopback!!!")
        ...
        out = self.vm0_testpmd.execute_cmd("stop")
        self.verify(
            self.vf0_mac.lower() not in dumpout, "Failed to enable tx loopback!!!"
        )
    ...
    def test_all_queues_drop(self):
        ...
        self.check_vf_link_status()
        self.vm1_testpmd.start_testpmd(
            VM_CORES_MASK, "--port-topology=chained --rxd=1024"
        )
        ...
        self.verify("RX-packets: 1023" in out, "Failed to let vf1 full of queues!!!")
    ...
    def test_vf_mac_set(self):
        ...
        self.verify(
            expect_mac.lower() in out, "Failed to receive packet on setted vf mac!!!"
        )
    ...
    def test_vlan_antispoof(self):
        ...
        self.verify(
            vf0_mac_lower in out, "Failed to disable vlan antispoof with match vlan!!!"
        )
        out = self.send_and_macstrip(self.vf0_mac, unmatch_vlan)
        self.verify(
            vf0_mac_lower in out,
            "Failed to disable vlan antispoof with unmatch vlan!!!",
        )
        out = self.send_and_macstrip(self.vf0_mac)
        self.verify(
            vf0_mac_lower in out, "Failed to disable vlan antispoof with no vlan!!!"
        )
        ...
        out = self.send_and_macstrip(self.vf0_mac, match_vlan)
        self.verify(
            vf0_mac_lower in out, "Failed to enable vlan antispoof with match vlan!!!"
        )

        out = self.send_and_macstrip(self.vf0_mac, unmatch_vlan)
        self.verify(
            vf0_mac_lower not in out,
            "Failed to enable vlan antispoof with unmatch vlan!!!",
        )

        out = self.send_and_macstrip(self.vf0_mac)
        self.verify(
            vf0_mac_lower not in out, "Failed to enable vlan antispoof with no vlan!!!"
        )
    ...
    def test_vlan_filter(self):
        ...
        self.verify("dst=%s" % wrong_mac in out, "Failed to receive untagged packet!!!")
        random_vlan = random.randint(1, MAX_VLAN)
        out = self.send_and_pmdout(wrong_mac, random_vlan)
        self.verify(
            "dst=%s" % wrong_mac in out, "Failed to receive packet with vlan id!!!"
        )
        self.verify(
            "VLAN tci=%s" % hex(random_vlan) in out,
        ...
        for rx_vlan in rx_vlans:
            ...
            self.verify("dst=%s" % wrong_mac in out, "Failed to enable vlan filter!!!")
            self.verify(
                "VLAN tci=%s" % hex(rx_vlan) in out,
                "Failed to receive packet with vlan id!!!",
            )
            ...
            self.verify(
                "dst=%s" % wrong_mac not in out, "Failed to enable vlan filter!!!"
            )
            self.dut_testpmd.execute_cmd("rx_vlan rm %s port 0 vf 1" % rx_vlan)
            time.sleep(3)
            out = self.send_and_pmdout(wrong_mac, rx_vlan)
            self.verify("dst=%s" % wrong_mac in out, "Failed to disable vlan filter!!!")
            self.verify(
                "VLAN tci=%s" % hex(rx_vlan) in out,
                "Failed to receive packet with vlan id!!!",
            )
            ...
            self.verify("dst=%s" % wrong_mac in out, "Failed to disable vlan filter!!!")
            self.verify(
                "VLAN tci=%s" % hex(wrong_rx_vlan) in out,
                "Failed to receive packet with vlan id!!!",
            )
        out = self.send_and_pmdout(wrong_mac)
        self.verify("dst=%s" % wrong_mac in out, "Failed to receive untagged packet!!!")
    ...
    def test_ixgbe_vf_jumboframe(self):
        ...
        self.vm0_testpmd.execute_cmd("stop")
        self.verify("received" not in out, "Failed to receive this length packet!!!")
        ...
        out = self.send_and_pmdout(self.vf0_mac, 0, pktsize)
        self.vm0_testpmd.execute_cmd("stop")
        self.verify("received" in out, "Failed to receive this length packet!!!")
    ...
    def test_stats_show_clear(self):
        ...
        out = self.dut_testpmd.execute_cmd("show vf stats 0 0")
        self.verify(
            "RX-packets: 10" in out and "TX-packets: 10" in out,
