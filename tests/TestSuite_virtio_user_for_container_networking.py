...
class TestVirtioUserForContainer(TestCase):
    ...
    def get_core_mask(self):
        ...
        self.verify(
            len(core_list) >= (self.nb_cores * 2 + 2),
        ...
        self.core_list_virtio_user = core_list[
            self.nb_cores + 1 : self.nb_cores * 2 + 2
        ]
    ...
    def check_2M_env(self):
        hugepage_size = self.dut.send_expect(
    ...
    def launch_testpmd_as_vhost(self):
        ...
        eal_param = self.dut.create_eal_parameters(
            ...
            prefix="vhost",
            vdevs=["net_vhost0,iface=vhost-net,queues=%d,client=0" % self.queue_number],
            ports=[self.pci_info],
        )
        if self.check_2M_env:
            eal_param += " --single-file-segments"
        command_line_client = (
            self.app_testpmd_path + eal_param + " -- -i --nb-cores=%d" % self.nb_cores
        )
